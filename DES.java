...
public class DES {
    ...
    private int[] PC1 = { 57, 49, 41, 33, 25, 17, 9, 1, 58, 50, 42, 34,
    ...
    private int[] expandTbl = { 32, 1, 2, 3, 4, 5, 4, 5, 6, 7, 8, 9, 8,
    ...
    private String[] intermediates = new String[67];
    ...
    private byte[] rotLeft(byte[] input, int len, int pas) {
    ...
    private byte[] extractBits(byte[] input, int pos, int n) {
    ...
    private byte[] permutFunc(byte[] input, int[] table) {
    ...
    private byte[] encrypt64Bloc(byte[] bloc,byte[][] subkeys, boolean isDecrypt) {
    ...
    private byte[] f_func(byte[] R, byte[] K) {
        byte[] tmp;
    ...
    private byte[] separateBytes(byte[] in, int len) {
    ...
    public String[] encryptAndReturnIntermediatesArray(byte[] data, byte[] key) {
        int lenght=0;
        byte[] padding = new byte[1];
        int i;
        lenght = 8 - data.length % 8;
        ...
        intermediates[idx] = byteArrayToBits(data);
        ++idx;
        for (i = 1; i < lenght; i++)
            padding[i] = 0;
    ...
    private String byteArrayToBits(byte[] byteArray) {
        String bitString = "";

        for (int i=0; i<byteArray.length; i++) {
            bitString = bitString.concat(byteToBits(byteArray[i]));
        }
    ...
    private int compareBitStrings(String original, String encoded) {
        int count = 0;
        ...
        if(original.length() < encoded.length()){
            int diffInLength = encoded.length() - original.length();
            original = String.format("%0"+diffInLength+"d", original);
            for  (int i = 0; i < original.length()-1; i++) {
                if (original.charAt(i)!=encoded.charAt(i)){
                    ++count;
                }
    ...
    public static void main(String[] args) {
        ...
        try
        {
            String textClar = "ABCDEF";
            String updated = "BBCDEF";
            //DES
            String k = "randomchars";
            ...
            for (int i = 0; i < 16; i++) {
                ...
                for (int j = 0; j < 4; j++) {
                    switch (j){
                        ...
                        case 1:
                            System.out.println("XoR Function:");
                            break;

                        case 2:
                            System.out.println("S-Box Operations:");
                            break;

                        case 3:
                            System.out.println("Permutation Functions (P):");
                            break;
                    }

                    System.out.println("\t\tOutput length:"+original[counter].length());
            ...
            System.out.println("---------------------------------------------------------------------------------------");
            System.out.println("Inverse Initial Permutation (invIP):");
            System.out.println("\t\tOutput length:"+original[counter].length());
